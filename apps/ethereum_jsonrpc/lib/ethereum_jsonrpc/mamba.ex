defmodule EthereumJSONRPC.Mamba do
  @moduledoc """
  Ethereum JSONRPC methods that are only supported by [Mamba](https://github.com/input-output-hk/mamba-world).
  """

  import EthereumJSONRPC, only: [id_to_params: 1, integer_to_quantity: 1, json_rpc: 2, request: 1]

  alias EthereumJSONRPC.{FetchedBalance, FetchedCode, Transactions}
  alias EthereumJSONRPC.Mamba.{Calls, Tracer}

  @behaviour EthereumJSONRPC.Variant

  @doc """
  Block reward contract beneficiary fetching is not supported currently for Mamba.
  ...
  @doc """
  Internal transaction fetching for entire blocks is not currently supported for Mamba.
  ...
  @doc """
  Fetches the pending transactions from the Mamba node. Temporary disabled
  """
  @impl EthereumJSONRPC.Variant
  def fetch_pending_transactions(json_rpc_named_arguments) do
    with {:ok, transaction_data} <-
      ...
      transactions_params =
        ...
        |> Enum.map(fn t -> Map.merge(%{"to" => nil, "v" => -1, "gasPrice" => t["maxFeePerGas"] || -1}, t) end)
